...
void subhandler(igris::buffer message, int index) 
{
	...
	frame[index] = cv::imdecode({message.data(), (int)message.size()}, cv::IMREAD_GRAYSCALE);
	isready[index] = true;
...
void routine_draw()
{
	...
	for (int i = 0; i < 2; ++i) {
		texture[i].reference_buffer(
			frame[i].data, 
			frame[i].size().width, 
			frame[i].size().height, GL_RED, GL_RED, GL_UNSIGNED_BYTE);
		texture[i].bind();
	}
	

	std::vector<std::pair<linalg::vec<float, 3>, linalg::vec<float, 2>>> vertices1 = {
	...
	std::vector<std::pair<linalg::vec<float, 3>, linalg::vec<float, 2>>> vertices2 = {
	...
	drawer.draw_grayscale_texture(vertices1, indices, texture[0], linalg::identity);
	drawer.draw_grayscale_texture(vertices2, indices, texture[1], linalg::identity);
...
int main(int argc, char** argv)
{
	...
	if (argc < 3)
	{
		nos::println("Usage: viewer LEFT_EYE_ADDR RIGHT_EYE_ADDR");
		exit(0);
	}

	auto addr1 = crow::address(argv[1]);
	auto addr2 = crow::address(argv[2]);
	crow::start_spin();

	while (1)
	{
		subs1.subscribe(1, addr1);
		subs2.subscribe(1, addr2);
